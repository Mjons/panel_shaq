...
- **"More ways to earn" now appears on the card.** Panel Haus recently made several things pay points — every credit you spend (1 each, uncapped), finishing a comic, saving a blueprint, shipping a meme, ordering a print — and mobile was already *paying* all of them, because the ledger is shared and the server does the awarding. It just wasn't showing them, so your points could climb with nothing on screen explaining why. The list is read-only and sits outside the numbered asks on purpose: those four drive the pips and the tier, and the tier is what the mint allowlist is ranked on, so nothing here can move your place in it.
- **That list says where the points come from, deliberately.** Only the per-credit award is earned inside this app; the rest are checked against Panel Haus's own projects, vault and orders, which mobile activity doesn't write — your comics live on your device. They're still listed because doing any of them on panelhaus.app pays into this same balance, but the card says so rather than implying "Finish a comic +50" is something this app can grant you.
- **The app installs ~5MB lighter than it would have.** The Creator Card ships a pool of 16 scenes and deals you exactly one, but the offline cache was set to precache every image in the app, so an install downloaded all sixteen to show you one. They're excluded now and fetched on demand instead, with the scene you're actually dealt cached the first time it renders, so it still works offline. Precache went from 23.1MB to 17.9MB.
- **You can change your mint reminder email after signing up for it.** Previously, once you opted in, the field locked and the only option was to turn the reminder off entirely. That address is the single one the mint notification goes to, so a typo would have been permanent for the person it matters most to. The field now stays editable with an **Update** button, and the "you're on the list" confirmation shows once it's saved. Matches the desktop card.
- **The desktop-only tags now appear on every desktop-only reward**, including ones you've already earned. They were hidden on earned rows, which made the list look arbitrary: "Finish a comic" sat untagged right next to a tagged "Complete a 3 page comic", implying the first one worked on your phone. It doesn't. Where something can be done doesn't stop being true once you've done it, so on an earned row the tag now simply reads as "you did this on desktop".
- **Every desktop-only earner is now labelled, and there's a "How to earn more points" explainer.** Each of those rows carries a small **DESKTOP** tag, because most people never open a help sheet and an unlabelled row reads as something you can do here. Tapping the **?** beside the list opens a breakdown split two ways: what you earn on your phone (every credit you spend, at one point each with no cap, plus the four asks) and what only counts on panelhaus.app, with the reason in plain words — those are checked against work saved to your Panel Haus account, and comics made on your phone stay on your phone. It also says the thing that matters most: your card tier, and your place in the mint queue, come only from the four asks, and all four work here. The rest are bonus points.
