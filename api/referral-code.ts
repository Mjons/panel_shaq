...
// Read-only proxy to Panel Haus's referral-code endpoint. Forwards the caller's
// Clerk Bearer; PH returns (and lazily generates) the user's own code + canonical
// referral URL. Mirrors api/credits-balance.ts.
//
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const r = await fetch(`${PH_BASE}/api/referral/code`, {
    ...
    return res.status(502).json({ error: "Failed to fetch referral code" });
  }
}
