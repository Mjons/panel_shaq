...
### Smudge — agentic chat comic builder (`smudge` tab)

`SmudgeScreen` (the first tab) builds comics through conversation: tell Smudge what you want, it plans the page, you confirm, and panels stream in. It's a **new front door, not a rebuild** — it writes into the same `rawPanels` / `pages` / `vaultEntries` the manual tabs use, so its output is fully hand-editable. Full plan + reasoning: `documents/MOBILE_AGENT_HANDOFF.md`. The pieces:

- **`src/services/agent/`** — the brain, all framework-agnostic and pure where possible. `mobileDigest.ts` projects the six-value document into the small typed shape the model reads (Smudge never sees storage; desktop's graph edges are our FK fields) and ports desktop's uncast-character check. `buildComic.ts` = `validatePlan` (over-limit is an ERROR the model is told about, never a silent `.slice()`) + `applyPlan` (pure: resolves cast/location NAMES to vault ids, reports `unresolved` rather than inventing, dialogue becomes `bubbles[]` and NEVER enters the art prompt, layout via the existing `getDefaultLayoutId`). `drawPanels.ts` composes the image prompt **mirroring `DirectorScreen.tsx:1530-1622`** and reuses the exported `generatePanelImage` (so ink/auth/out-of-ink are handled identically to manual generation). `turnSnapshot.ts` is the one-level undo (snapshots the three arrays before a turn/commit; keeps base64 `image` by reference, not cloned). `runTurn.ts` is the turn loop (rebuild digest every round, `MAX_ROUNDS` 4, never end empty). `agentClient.ts` is the transport — deliberately **not** `apiPost`, because `apiPost`'s per-call out-of-ink precheck would abort an already-paid multi-round turn at round 2; the gate runs **once** in `preflightTurn`.
- **`api/board-agent.ts`** — the relay. Self-contained per the route rules (inlined helpers, the **system prompt is server-authoritative and inlined**; the tool manifest is client-sent because the client executes the tools). Gemini-only. **One charge per turn** via PH's idempotent reserve keyed on `turn_<turnId>` (no Redis, unlike desktop; `newBalance` is surfaced only on the real charge, not idempotent replays, so `ink_spent` fires once). Two hard Gemini-3 traps are handled: `thoughtSignature` must round-trip verbatim (a dropped one is a hard 400 on the next tool round), and the digest is folded into `system`, never sent as a trailing user turn after a `functionResponse`. No separate free-tier gate is needed: charging ink IS the gate (there are no free turns to farm; free-tier users spend granted ink like they do for images).
- **`useSmudgeChat.ts`** owns the conversation and backs the transcript + doc with refs so the loop reads live state across rounds. Draft-first (the locked product decision): `build_comic` produces a `PendingDraft` that only commits on **Keep it**; each commit and each in-turn edit is one **Undo that**. Drawing (`drawAllPending`) fills art-less committed panels one at a time. `useVoiceInput.ts` is feature-detected hold-to-talk (Web Speech API; the mic only renders where supported + secure).
- **Env:** `INK_COST_AGENT_TURN` (default 1) and `MOBILE_AGENT_GEMINI_MODEL` (default `gemini-3.1-flash-lite-preview`). The five tools (`build_comic`, `update_panel`, `set_dialogue`, `save_character`, `suggest_next`) are deliberately few — desktop has 37; each tool is context paid for every round.
...
**Crypto rail (OxaPay):** the same sheet also sells boosters for crypto — `startCryptoBoosterCheckout` → `api/crypto-create-invoice` (a byte-identical twin of `credits-checkout`, only the upstream path differs) → PH `crypto/create-invoice` → OxaPay hosted page. Four things are load-bearing: (1) **never build a webhook here** — OxaPay bakes `callback_url` into each invoice and PH pins it to its own canonical host, so pointing it at `m.panelhaus.app` would take the money and never credit it (a trap caught in PH's design review, changelog `1275`); (2) no OxaPay keys, prices or credit amounts live here (PH reads the USD amount live from Stripe so the two rails can't disagree; `PACKS` is display-only); (3) the Card/Crypto toggle renders **only** when PH's balance payload reports `cryptoEnabled` (surfaced through `fetchAccount`), never from a local flag — no dead buttons; (4) crypto settles on-chain, so the `/success?crypto=1` return polls `CRYPTO_POLL_DELAYS_MS` (~3 min) and ends on an honest "still confirming" message, while the card path keeps its 3s/7s webhook timing. PH rejects `type: "subscription"` on purpose (crypto can't auto-renew) and rate-limits invoices 5/min/user. Spec: `documents/PANEL_SHAQ_CRYPTO_CHECKOUT_HANDOFF.md`.

**Testing the crypto rail locally is awkward, and a `503` there is usually NOT our bug.** Both sides must sit on the same Clerk instance, so with `.env.local` on `pk_test`/`sk_test` you must point at a **locally running PH** (`PANELHAUS_API_BASE=http://localhost:3001`); aiming at production PH `401`s because a dev-instance token won't verify against the live one. That local PH then typically fails its own config gate with `503 {code:"SANDBOX_IN_PROD"}` — PH refuses `OXAPAY_SANDBOX=true` whenever `isProduction()` is true, and under `vercel dev` the function process reports production regardless of `VERCEL_ENV=development`. Unblock it with `OXAPAY_SANDBOX_ALLOW_PROD=true` in **PH's** `.env.local`, knowing sandbox invoices mint **real** credits into the shared ledger. Note the toggle can still render while invoice creation is blocked: PH's `cryptoEnabled` flag is only `isOxapayEnabled() && isOxapayConfigured()` and omits the sandbox/`ENABLE_PAYMENTS` gates (PH-side inconsistency, harmless in prod). To identify which `503` you hit, POST to the PH route with a junk Bearer: the config gates run **before** auth, so a `401` back means config is healthy.
...
`src/services/analytics.ts` is the single funnel: `track()`/`trackOnce()` **fan out to BOTH Vercel Analytics and PostHog**. `@vercel/analytics` is mounted via `<Analytics/>` in `src/main.tsx` (always on); **PostHog** (`posthog-js`) is initialized by `initAnalytics()` in `main.tsx`, **gated on `VITE_POSTHOG_KEY`** (unset → PostHog is a no-op, Vercel still runs). Because every event flows through `track()`, existing events (`generation_started {type}`, `share_completed`, etc.) reach PostHog automatically. PostHog also gets: `identifyUser`/`resetUser` via `<PosthogIdentifyBridge/>` (Clerk sign-in → identify with email/wallet + auth_method; sign-out → reset), `setUserProps({tier})` from `credits.ts`, and targeted events (`signed_up`, `ink_spent`, `out_of_ink`, `checkout_started`, `purchase_completed`, `referral_shared`). Config: US host, `capture_pageview:'history_change'`, `person_profiles:'identified_only'`, and **autocapture + session recording OFF**. Adding analytics is fire-and-forget; don't block UX on it. See `documents/POSTHOG_ANALYTICS.md`.

**We share ONE PostHog project with MemeGen** (Panel Haus desktop is not on PostHog at all — it uses Google Analytics `gtag`). Three things make that safe, and breaking any of them corrupts the shared data: (1) `<PosthogIdentifyBridge/>` identifies as **`clerk:${user.id}`** because MemeGen does the same (`MemeGen/src/lib/AuthContext.jsx`) — same Clerk instance, so the prefix is what makes one human ONE person across both apps. It also averts a real conflict: PostHog's cookie is cross-subdomain on `.panelhaus.app`, so a MemeGen-identified visitor already arrives carrying `clerk:<id>`, and PostHog refuses to merge two *identified* ids. (2) `initAnalytics()` calls `posthog.register({ app: "panel_shaq", env })` so every event is separable from MemeGen's `app:"memegen"` + `whitelabel:<brand>`; super properties live in per-origin localStorage (the shared cookie carries only `distinct_id`/`$device_id`/session keys), so they can't leak between apps. (3) autocapture and session recording stay **off** — one project means one event/recording allowance, and MemeGen disables both too. Set `VITE_POSTHOG_KEY` to MemeGen's project key.
