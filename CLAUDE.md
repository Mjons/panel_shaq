...
- `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` — the GTD ship-claim store (`api/creator-application`). Use the **same values as Panel Haus** so both apps share one dedupe namespace (`creator:application:<identity>`) and one admin list (`creator:applications`). Unset → the route fails open (claims accepted but dropped, never 500). Optional overrides (defaults match PH): `CREATOR_APP_RATE_LIMIT_PER_HOUR=10`, `CREATOR_APP_MAX_CHARS=4000`, `CREATOR_APP_LIST_MAX=20000`, `CREATOR_APP_KEY_TTL_DAYS=365`.
